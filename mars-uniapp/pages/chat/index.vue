          ...
          <view class="bubble-box">
            ...
            <view class="msg-bubble bubble-other" @longpress="handleMsgLongPress(msg)">
              ...
              <view v-else-if="msg.msgType === 3" class="msg-file" @tap="previewFile(msg)">
                ...
                <text class="file-name">{{ msg.content }}</text>
              </view>
            </view>
          </view>
        </view>
...
<script>
import {getChatHistory, sendMessage, markAsRead, uploadFile} from '../../utils/api.js'
...
export default {
  data() {
    return {
      targetId: 0, targetName: '', targetAvatar: '', myUserId: 0, myAvatar: '', myName: '',
  ...
  methods: {
    ...
    async sendImageFile(filePath) {
      ...
      try {
        ...
        await sendMessage({receiverId: this.targetId, content: url, msgType: 2})
        this.messages.push({
          id: Date.now(), senderId: this.myUserId, senderAvatar: this.myAvatar,
          receiverId: this.targetId, content: url, msgType: 2, sendTime: new Date().toISOString()
        })
        this.$nextTick(() => this.scrollToBottom())
      } catch (e) {
    ...
    async loadMessages() {
      try {
        const res = await getChatHistory(this.targetId, 1, 50)
    ...
    async loadMoreHistory() {
      ...
      this.page++
      try {
        const res = await getChatHistory(this.targetId, this.page, 50)
    ...
    async handleSend() {
      ...
      const tmp = {
        id: Date.now(), senderId: this.myUserId, senderAvatar: this.myAvatar,
        receiverId: this.targetId, content: c, msgType: 1, sendTime: new Date().toISOString()
      }
      this.messages.push(tmp);
      this.$nextTick(() => this.scrollToBottom())
      try {
        const res = await sendMessage({receiverId: this.targetId, content: c, msgType: 1})
    ...
    setupWebSocket() {
      this.chatHandler = (data) => {
        if (String(data.senderId) === String(this.targetId)) {
          this.messages.push({
            id: Date.now(), senderId: data.senderId, senderName: data.senderName,
            senderAvatar: this.targetAvatar, receiverId: this.myUserId, content: data.content,
    ...
    removeWebSocketListeners() {
      if (this.chatHandler) wsClient.off('chat', this.chatHandler)
    },
    markRead() {
      markAsRead(this.targetId).catch(() => {
      })
    },
    scrollToBottom() {
      this.scrollToId = '';
    ...
    handleMsgLongPress(msg) {
      uni.showActionSheet({
        ...
        success: (r) => {
          if (r.tapIndex === 0) uni.setClipboardData({data: msg.content})
        }
      })
    }
  }
